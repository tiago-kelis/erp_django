...
#     message = 'O funcionário não têm permisão para gerenciar os status das tarefas'
    
#     def has_permission(self, request, _view) -> bool | None:
#         return Check_permissions(request.user, request.method, permission_to='task_status')
    
# class TaskTypePermission(permissions.BasePermission):
#     message = 'O funcionário não têm permisão para gerenciar os tipos das tarefas'
    
#     def has_permission(self, request, _view) -> bool | None:
#         return Check_permissions(request.user, request.method, permission_to='task_type')   
    
# class TaskPriorityPermission(permissions.BasePermission):
#     message = 'O funcionário não têm permisão para gerenciar as prioridades das tarefas'
    
#     def has_permission(self, request, _view) -> bool | None:
#         return Check_permissions(request.user, request.method, permission_to='task_priority')